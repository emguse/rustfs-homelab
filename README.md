...
.envで登録した アクセスキーとシークレットキーによるログインは常用せず、 admin-equivalent などのユーザーを作成した上で適切な権限(readwrite等)を付与して管理してください。  
実際のバケットへのアクセスついても、サービスごとの限定アクセスポリシーとユーザーを作成して使用してください。
